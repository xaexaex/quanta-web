    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-gray-100 bg-background/90 backdrop-blur-xl supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto px-4 sm:px-6 h-20 md:h-24">
        <div className="flex items-center justify-between h-full">
          {/* Logo */}
          <Link href="/" className="text-2xl md:text-3xl font-bold tracking-tighter text-foreground flex-shrink-0">
            ...
            {navItems.map((item) => (
              ...
              >
                <button className={`text-sm font-semibold transition-colors flex items-center gap-1 cursor-pointer ${
                  activeDropdown === item.name ? 'text-primary' : 'text-muted hover:text-primary'
                }`}>
                  {item.name}
