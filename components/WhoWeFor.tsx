...
function CompanyLogo({ company }: { company: typeof AI_COMPANIES[0] }) {
  ...
  return (
    <div className="w-12 h-12 rounded-xl bg-gray-100 border border-[rgba(0,0,0,0.08)] flex items-center justify-center overflow-hidden shrink-0 shadow-inner">
      {!error && company.domain ? (
        ...
        <span
          className="text-sm font-black text-[#C4ED5F]"
  ...
  return (
    <section className="py-16 sm:py-28 bg-transparent relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16 text-center">
        ...
        <h2
          className="text-5xl sm:text-6xl md:text-7xl font-bold text-black leading-[0.95] tracking-tight mt-2 mb-6"
          style={{ fontFamily: "var(--font-syne)" }}
        >
          The AI Execution
          <br />
          <span className="text-[#C4ED5F]">Infrastructure Layer.</span>
        </h2>
      </div>
      ...
      <div className="relative overflow-hidden flex flex-col gap-6 py-6">
        ...
        <div className="flex animate-marquee gap-6 w-max">
          {marqueeItems1.map((company, i) => (
            <div
              key={`row1-${i}`}
              className="flex items-center gap-5 px-8 py-5 rounded-2xl border border-[rgba(0,0,0,0.06)] bg-gray-50 shadow-[0_0_20px_rgba(0,0,0,0.5)] whitespace-nowrap flex-shrink-0 group hover:border-[rgba(196,237,95,0.3)] hover:bg-gray-50 transition-all duration-300 cursor-default"
            >
              <CompanyLogo company={company} />
              <div>
                <p className="text-lg font-bold text-black leading-none tracking-wide">{company.name}</p>
                <p
                  className="text-xs text-gray-600 mt-1.5 font-medium tracking-widest uppercase"
                  style={{ fontFamily: "var(--font-mono)" }}
                >
                  {company.label}
                </p>
