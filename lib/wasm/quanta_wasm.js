/* @ts-self-types="./quanta_wasm.d.ts" */

/**
 * Compute the canonical signing hash for a given payload (debugging helper).
 * Returns 32-byte hash as lowercase hex.
 * @param {string} tx_data_hex
 * @returns {string}
 */
export function compute_signing_hash(tx_data_hex) {
    let deferred3_0;
    let deferred3_1;
    try {
        const ptr0 = passStringToWasm0(tx_data_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.compute_signing_hash(ptr0, len0);
        var ptr2 = ret[0];
        var len2 = ret[1];
        if (ret[3]) {
            ptr2 = 0; len2 = 0;
 ...
 * @returns {string}
 */
export function get_address(pubkey_hex) {
    let deferred3_0;
    let deferred3_1;
    try {
 ...
 * @returns {any}
 */
export function import_wallet(mnemonic_phrase, passphrase, index) {
    const ptr0 = passStringToWasm0(mnemonic_phrase, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passStringToWasm0(passphrase, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.import_wallet(ptr0, len0, ptr1, len1, index);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
 ...
 * @returns {string}
 */
export function sign_transaction(tx_data_hex, secret_key_hex) {
    let deferred4_0;
    let deferred4_1;
    try {
        const ptr0 = passStringToWasm0(tx_data_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
 ...
 * @returns {boolean}
 */
export function verify_signature(hash_hex, signed_msg_hex, pubkey_hex) {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('quanta_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
